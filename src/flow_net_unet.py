...
"""使用 UNet 结构的 Flow Matching Policy"""
...
class ConditionalResidualBlock1D(nn.Module):
    """Conditional residual block with FiLM 调制."""
...
class ConditionalUnet1D(nn.Module):
    """用于 diffusion model 的带 conditioning 的 1D U-Net。"""
    ...
    def initialize_layers(self, init_fn=None):
        """
        初始化 all layers in the U-Net with a given initialization function.
...
class FlowMatchingUnetModel(nn.Module):
    """用于行为克隆的 UNet 版 Flow Matching model。"""
    ...
    def forward(self, x_t: Tensor, t_emb: Tensor, obs_cond: Tensor) -> Tensor:
        ...
        """
        # Pass through UNet (it expects timeembedding and global_cond 分别)
    ...
    def initialize_layers(self, init_fn=None):
        """
        初始化 all layers in the UNet with a given initialization function.
