...
#
# 启动全部 60 个主 benchmark run：
#   5 tasks x 4 models (FPO++, DPPO Learned, Vanilla FPO, DPPO Fixed) x 3 seeds
#
# 用法：
#   bash scripts/run_main_benchmark.sh              # 执行全部 run
#   DRY_RUN=1 bash scripts/run_main_benchmark.sh    # 只打印命令
#   NUM_GPUS=4 bash scripts/run_main_benchmark.sh   # 每个 run 使用 4 张 GPU
#
# 各 run 会顺序启动。若要并行运行，可将 DRY_RUN=1 输出传给
# 作业调度器，或使用 & 将单个 run 放到后台。
