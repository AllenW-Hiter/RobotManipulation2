...
class TrainFlowBCConfig:
    """配置 for offline training on a HF Hub dataset with LeRobot policies."""
...
def _annotate_frame(
    ...
    # 添加 text annotation to the frame
    pil_img = Image.fromarray(frame)
    draw = ImageDraw.Draw(pil_img)

    # 准备 episode status text
...
def _run_rollouts(
    *,
    ...
    """在向量化 env 中用 policy 运行 num_episodes 个 episode 并计算成功率。

    Captures a video, writes it to *save_dir*/`eval_step_<step>.mp4`, and 返回 `(success_rate, video_path)`.
    """
    ...
    # 创建 video writer at the beginning
    from datetime import datetime

    now = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    # 使用 the provided save_dir for videos
    save_dir.mkdir(parents=True, exist_ok=True)
...
def save_checkpoint(checkpoint_dir: Path, step: int, policy: PreTrainedPolicy, optimizer):
    """保存 checkpoint with model and optimizer state."""
...
def load_checkpoint(checkpoint_dir: Path, policy: PreTrainedPolicy, optimizer, load_ema: bool = False, device: str = "cuda"):
    ...
    # 加载 optimizer state and EMA if available
    ...
    if optimizer_path.exists():
        ...
        # 加载 EMA state if available and policy has EMA
        if hasattr(policy, 'ema_model') and policy.ema_model is not None:
...
def download_checkpoint_from_wandb(run_id: str, project: str, entity: str, artifact_alias: str = "latest", download_dir: Path = Path("./downloaded_checkpoints")) -> Path:
    ...
    # IMPORTANT: 获取 artifacts FROM THE SPECIFIC RUN, not from the project level
    ...
    def get_step_from_artifact(artifact):
        """从 artifact metadata 或名称中提取 step 编号."""
        # Try metadata first
    ...
    # 显示全部可用 checkpoint for debugging
    checkpoint_artifacts.sort(key=get_step_from_artifact, reverse=True)
...
def main(cfg: TrainFlowBCConfig):
    ...
    # 创建 run directory structure to organize all outputs
    from datetime import datetime

    run_start_time = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

    # 设置输出目录 - organize everything under a single run directory
    ...
    # 启用 geometric augmentations (rotation, translation, scale)
    # 设置 weight to 1.0 to enable, 0.0 to disable
    # Note: Geometric augmentations are disabled by default
    # You can also customize the 参数 in transforms.py or directly in pretrain_flow_bc.py:
    ...
    # 添加 resize to 224x224 if using CLIP as vision backbone
    ...
    # ---------------------------------------------------------------------
    # 加载 checkpoint weights if resuming
    # ---------------------------------------------------------------------
    start_step = 0
    ...
    if cfg.rollout_freq and cfg.eval_env:
        # ------------------------------------------------------------------
        # 创建 the DexMimicGen evaluation environment
        # ------------------------------------------------------------------
        ...
        if cfg.eval_env in envs:
            # 创建 DexMimicGen environment (vectorized or single based on debug flag)
            if rank == 0:
    ...
    while step < cfg.steps:
        ...
        for i in range(cfg.gradient_accumulation_steps):
            if is_ddp:
                # 设置 epoch to ensure different shuffling for each micro-batch
            ...
            # 保存 sample images for inspection (only once at step 0)
            if cfg.debug and step == 0 and rank == 0:
                ...
                for key in batch.keys():
                    if "observation.images" in key:
                        ...
                        # 保存 first 8 images from the batch as a grid
            ...
            if is_ddp and not is_last_accumulation_step:
                with policy.no_sync():  # type: ignore
                    ...
                    # 平均 loss over accumulation steps
        ...
        # 计算 gradient norm before clipping (for logging)
        ...
        # 计算 fractional epoch (how many full passes over the dataset)
        ...
        if (step % cfg.save_freq == 0 and step != start_step) or step + 1 == cfg.steps:
            if rank == 0:
                ...
                # 保存 checkpoint to checkpoints directory
                ...
                if cfg.wandb_enable and wandb.run is not None:
                    try:
                        # 创建 and log checkpoint artifact
        ...
        if (
            ...
            # 启用 EMA for evaluation if configured
            ...
            # 禁用 EMA after evaluation (not needed since we deepcopied, but good practice)
            if isinstance(model_to_eval, PreTrainedPolicy):
            ...
            if cfg.wandb_enable:
                ...
                # 将视频作为 artifact 上传 and add to log data
                if video_path is not None and video_path.exists():
                    try:
                        # 将视频作为 artifact 上传 for versioned storage
                        ...
                        # 添加 video for UI visualization to the same log call
        ...
        # 所有 rank 必须等待 rank 0 to finish evaluation before proceeding
