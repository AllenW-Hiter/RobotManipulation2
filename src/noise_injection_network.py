...
"""用于 Flow Matching Policy 学习探索噪声的噪声注入网络。"""
...
class NoiseInjectionNetwork(nn.Module):
    ...
    def __init__(
        ...
        """
        初始化 the noise injection network.
        ...
        # 输出层: predict noise scale for each (horizon, action_dim) pair
        # Output shape 重塑后: (B, T, D)
        ...
        # 初始化 output layer 以产生 small initial noise
        self._init_output_layer()

    def _init_output_layer(self):
        """初始化 the output layer 以产生 conservative initial noise."""
        # 获取 the last linear layer
        for module in reversed(list(self.mlp.modules())):
            if isinstance(module, nn.Linear):
                # 初始化 以产生 noise close to min_noise after sigmoid scaling
                # sigmoid(bias) * (max - min) + min ≈ min_noise
                # We want sigmoid(bias) ≈ 0, so bias should be negative
                nn.init.zeros_(module.weight)
                # Start with small negative bias 以产生 low noise initially
                nn.init.constant_(module.bias, -3.0)
                break

    def forward(self, obs_cond: Tensor) -> Tensor:
        """
        前向传播 to predict noise injection scales.
        ...
        # 添加 singleton dimension: (B, T, D) -> (B, T, 1, D)
        # 这允许跨 flow sampling step 广播
